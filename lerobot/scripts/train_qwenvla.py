...
from lerobot.common.policies.qwenvla.modeling_qwenvla import extract_all_batch_images_and_captions 
...
def evaluate_validation_loss(policy, val_loader, device, use_amp=False, max_batches=10):
    ...
    with torch.no_grad(), (torch.autocast(device_type=device.type) if use_amp else nullcontext()):
        for i, batch in enumerate(val_loader):
            ...
            # 第一次拿到 policy 时初始化 mgr
            if mgr is None and hasattr(policy.model, "lang_cache_mgr"):
                mgr = policy.model.lang_cache_mgr

            if mgr is not None:
                # 如果上一次任务完成，就用这批数据重新 launch
                if mgr.future is None or mgr.future.done():
                    image_dict = {k: v for k, v in batch.items() if k.startswith("observation.image")}
                    image_lists, caption_list = extract_all_batch_images_and_captions(image_dict, max_views=4)
                    goal_texts = batch["task"]
                    mgr.launch(image_lists, caption_list, goal_texts)

                # **阻塞** 等最新这次 VLM 跑完
                mgr.wait()

                # 拿到最新的 lang_embs
                batch["lang_embs"] = mgr.lang_cache
...
def train(cfg: TrainPipelineConfig):
    ...
    # ---- LangCache Warmup（慢系统首次初始化）----
    if hasattr(policy, "model") and hasattr(policy.model, "lang_cache_mgr"):
    # if lang_mgr.future is None or lang_mgr.future.done():
        logging.info(colored("Warming up lang_cache via VLM...", "cyan"))
        # 拿一个 batch 数据做初始化
        warmup_batch = next(dl_iter)
        image_dict = {k: v for k, v in warmup_batch.items() if k.startswith("observation.image")}
        current_image_list, caption_list = extract_all_batch_images_and_captions(image_dict, max_views=4)
        current_goal = warmup_batch["task"]  # 你需要确认 batch["task"] 是字符串列表
        # policy.model.lang_cache_mgr.launch(current_image_list ,caption_list, current_goal)
        # import asyncio
        # asyncio.run(policy.model.lang_cache_mgr.ready.wait())
        # logging.info(colored("lang_cache is ready!", "cyan"))
        async def warmup():
            await policy.model.lang_cache_mgr._run_vlm(current_image_list, caption_list, current_goal)
        

        import asyncio
        asyncio.run(warmup())
        print(policy.model.lang_cache_mgr.lang_cache.shape)
        logging.info(colored("lang_cache is ready!", "cyan"))
    ...
    for _ in range(step, cfg.steps):
        ...
        # -------- Slow system: launch VLM async update --------
        if hasattr(policy, "model") and hasattr(policy.model, "lang_cache_mgr"):
            lang_mgr = policy.model.lang_cache_mgr
            # if lang_mgr.task is None or lang_mgr.task.done():
            if lang_mgr.future is None or lang_mgr.future.done():
                logging.info("SLOW LOOP: launching VLM")
                # 找出当前 batch 中的 N 个样本
                ...
                image_dict = {k: v for k, v in warmup_batch.items() if k.startswith("observation.image")}
                image_lists, caption_list = extract_all_batch_images_and_captions(image_dict, max_views=4)  # 返回 List[PIL.Image]
                
                goal_texts=batch["task"]  # 第 i 个任务描述

                # 获取 [B, L, D] 的 prefix embedding
                # prefix_embs = lang_mgr.vlm.get_batch_prefix_embeddings(image_lists, caption_list, goal_texts)
                # # 存到 batch 里，供后面的 fast loop 使用
                # batch["lang_embs"] = prefix_embs.to(device)
                lang_mgr.launch(image_lists, caption_list, goal_texts)


        # -------- Fast system: diffusion training --------
        logging.info("FAST LOOP: update_policy")
        lang_embs = policy.model.lang_cache_mgr.get_cache()
        assert lang_embs is not None, "第一次还没跑完，请先用 warmup 跑一次 VLM"
        batch["lang_embs"] = lang_embs
