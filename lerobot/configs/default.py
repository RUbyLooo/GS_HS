...
class SingleDatasetConfig:
    ...
    scene_embedding_pt: Optional[str] = None
    subtask_embedding_pt: Optional[str] = None
    image_transforms: ImageTransformsConfig = field(default_factory=ImageTransformsConfig)
...
class MixedDatasetConfig:
    is_mix: bool
    repo_id: str
    datasets: List[Union[SingleDatasetConfig, dict]]
    image_transforms: ImageTransformsConfig = field(default_factory=ImageTransformsConfig)
    video_backend: str = field(default_factory=get_safe_default_codec)
    revision: str | None = None
    def __post_init__(self):
        for i, ds in enumerate(self.datasets):
            if isinstance(ds, dict):
                self.datasets[i] = SingleDatasetConfig(**ds)
