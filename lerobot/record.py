...
def record_loop(
    ...
    time.sleep(0.5)  # 等待物理线程恢复
    

    timestamp = 0
    start_episode_t = time.perf_counter()
    while robot.env.current_episode_done is False:
        ...
        if robot.env.current_episode_done is True:
            break
...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    # robot = make_robot_from_config(cfg.robot, mode = "inference")
    robot = make_robot_from_config(cfg.robot, mode="record")
    ...
    for recorded_episodes in range(cfg.dataset.num_episodes):
        ...
        print(f"recorded_episodes : {recorded_episodes}")

        # 关闭物理线程
        robot.env.pause_physical_thread()
