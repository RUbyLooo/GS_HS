...
def resize_pair_to_256(pair):
    B, C, T, H, W = pair.shape
    pair = pair.permute(0, 2, 1, 3, 4)  # [B, T, C, H, W]
    pair = pair.reshape(B * T, C, H, W)  # [B*T, C, H, W]

    resized = resize_with_pad(pair, width=256, height=256)  # [B*T, C, 256, 256]

    resized = resized.reshape(B, T, C, 256, 256).permute(0, 2, 1, 3, 4)  # [B, C, T, 256, 256]
    return resized
...
class SmolVLAPolicy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        self.laq = laq = LatentActionQuantization(
            dim = 512,
            quant_dim=32,
            codebook_size = 8,
            image_size = 256,
        ...
        laq_ckpt = "/home/cfy/cfy/gs_hs_backup/gs_hs/lerobot/common/policies/egovla/laq/lap_ckph/vae.120000.pt"
        self.laq.load(laq_ckpt)
        self.laq.eval()
    ...
    def forward(self, batch: dict[str, Tensor], noise=None, time=None) -> dict[str, Tensor]:
        ...
        images, img_masks = self.prepare_images(batch)
        state = self.prepare_state(batch)                         # [batch, 32]
        lang_tokens, lang_masks = self.prepare_language(batch)    # [batch, token_num]
        ...
        tN_idx   = deltas.index(50) \
                    if (True and 50 in deltas) \
                    else None
        seq = batch["observation.images.top"]            # [B, T, C, H, W]
        img0 = seq[:, t0_idx]       # 当前帧
        imgN = seq[:, tN_idx]       # +offset 帧
        pair = torch.stack([img0, imgN], dim=2).to("cuda")  # [B, C, 2, H, W]
        pair = resize_pair_to_256(pair)

        with torch.no_grad():
            index_batch = self.laq(
                pair, return_only_codebook_ids=True
            )  # [B, chunk_size，query_seq_length, latent_dim] ([B,N,L,D])

            codes_token = self.laq.decode_from_codebook_indices(index_batch)


        losses, vlm_suffix_out = self.model.forward(images, img_masks, lang_tokens, lang_masks, state, actions, noise, time)
        latent_action_loss = F.mse_loss(codes_token, vlm_suffix_out, reduction="none")
        ...
        # For backward pass
        loss = losses.mean()
        latent_action_loss = latent_action_loss.mean()
        # For backward pass
        loss_dict["loss"] = loss.item() + latent_action_loss.item()
...
class VLAFlowMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.latent_action_out_proj = nn.Linear(960, 32)
    ...
    def forward(
        ...
        (vlm_suffix_out, suffix_out), _ = self.vlm_with_expert.forward(
        ...
        # 对 vlm 的输出重新映射至 latent action
        vlm_suffix_out = vlm_suffix_out.to(dtype=self.latent_action_out_proj.weight.dtype)
        vlm_suffix_out = vlm_suffix_out.permute(0, 2, 1)  # [B, dim=960, seq=197]
        vlm_suffix_out = F.adaptive_avg_pool1d(vlm_suffix_out, 196)  # 变成长度196（能被4整除）
        vlm_suffix_out = vlm_suffix_out.permute(0, 2, 1)  # [B, seq=196, dim=960]

        vlm_out_b, vlm_out_s, vlm_out_d = vlm_suffix_out.shape
        vlm_suffix_out = vlm_suffix_out.view(vlm_out_b, 4, vlm_out_s // 4, vlm_out_d)  # [B,4,49,960]

        vlm_suffix_out = vlm_suffix_out.mean(dim=2)  # 对49个切片做平均，变成 [B,4,960]

        vlm_suffix_out = self.latent_action_out_proj(vlm_suffix_out)  # nn.Linear(960, 32), 输出 [B,4,32]
