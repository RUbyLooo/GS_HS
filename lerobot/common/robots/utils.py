...
def make_robot_from_config(config: RobotConfig, mode: str = "record") -> Robot:
    if config.type == "single_piper":
        from .single_piper import SinglePiper
        return SinglePiper(config, mode)
    elif config.type == "dual_piper":
        from .dual_piper import DualPiper
        return DualPiper(config,mode)
    elif config.type == "moving_dual_piper":
        from .moving_dual_piper import MovingDualPiper
        return MovingDualPiper(config,mode)
    elif config.type == "sim_single_piper":
        from .sim_single_piper import SimSinglePiper
        return SimSinglePiper(config, mode)
    else:
